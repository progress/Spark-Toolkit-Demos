/*
 ...
 @license Apache-2.0
*/
(function(d,f,a,b){if(d&&f&&a&&"undefined"===typeof b.spark){var c=null,e=null,g=null,h=null,n={id:"",filterQuery:"",data:{},success:!1,error:!1},k=null;b.spark={version:"vPRE (2018.07.24.082016)",field:{},form:{},grid:{},loader:{},nav:{},notify:{},strings:{},jsdoError:function(a){var c=((a.xhr||{}).jsdo||{getErrors:function(){return[]}}).getErrors(),e=[];d.each(c,function(b,a){switch(a.type){case -1:var c=null;if(a.responseText)try{c=JSON.parse(a.responseText||"")}catch(r){console.log("Error while parsing response: "+
...
alert(e[0]));a.preventDefault();return!1},jsdoFailure:function(a){if(a.request&&a.request.xhr&&401===a.request.xhr.status||403===a.request.xhr.status)b.spark.jsdoAuthError?b.spark.jsdoAuthError(a.request):alert("Session has expired. Please login again.");else if(a.request&&a.request._errors&&0<a.request._errors.length){var c="",e,g=a.request.response._errors.length,l;for(e=0;e<g;e+=1)l=a.request.response._errors[e]||{},c=c+" "+l._errorMsg||"UNKNOWN";""!==d.trim(c)&&console.warn(c)}},checkAuthentication:function(b,
d){h||(h=new a.data.AuthenticationProvider({uri:b,authenticationModel:d||a.data.Session.AUTH_TYPE_ANON}));var g=c||"anonymous",m=e||"anonymous";return h.hasClientCredentials()?h.logout().then(function(){return h.login(g,m)}):h.login(g,m)},createSession:function(l,m,n,f){g||(k||(f?k=f:this.setSessionName(l)),h||(h=new a.data.AuthenticationProvider({uri:l,authenticationModel:n||a.data.Session.AUTH_TYPE_ANON})),g=new a.data.JSDOSession({authenticationModel:n||a.data.Session.AUTH_TYPE_ANON,authProvider:h,
serviceURI:l||null,name:k||"JSDOSession_Default"}));if(m)return g.loginResult===a.data.Session.LOGIN_SUCCESS?g.isAuthorized().then(function(a,c,d){return b.spark.loadCatalogs(m)},function(a,c,d){return b.spark.loadCatalogs(m)}):h.login(c||"anonymous",e||"anonymous").then(function(a,c,d){return b.spark.loadCatalogs(m)},function(){return"Unable to reach the REST adapter to establish a session."});l=d.Deferred();l.resolve("No catalog(s) requested.");return l},loadCatalogs:function(a){g.catalogLoaded=
{};"string"==typeof a?g.catalogLoaded[a]=!1:a instanceof Array&&d.each(a,function(a,b){g.catalogLoaded[b]=!1});return g.addCatalog(a).then(function(a,b,c){d.each(c,function(a,b){g.catalogLoaded[b.catalogURI]=b.result?!0:!1});return c},function(a,b,c){console.log("Response:",c);return"Failed to load catalog(s)."})},createJSDO:function(b){var c=null;d.each(g.JSDOs,function(a,d){d.name===b&&(c=d)});return c||new a.data.JSDO({name:b})},getMessages:function(a){a.response&&(a=a.response);return a&&a._errors&&
0<a._errors.length?a._errors||!1:!1},showMessage:function(a,b){var c=a._errorMsg||"",d;switch(a._errorType.toLowerCase()){case "error":case "fatal":case "trace":d="error";break;case "validation":case "warning":d="warning";break;default:d="info"}""!==c&&d&&b&&b(c,d)},setCredentials:function(a,b){c=a||null;e=b||null},setSessionName:function(a){k=a.replace(/\//g,"");""===k&&(k="Root");return k="JSDOSession_"+k},clone:function(a,b){return d.extend(!0,{},a,b)},toProperCase:function(a){return a.replace(/\w\S*/g,
function(a){return a.charAt(0).toUpperCase()+a.substr(1).toLowerCase()})},getJsdoSession:function(){return g},getMergedOptions:function(a){return this.clone(n,a)},getQueryStringValue:function(a){var c=b.location.search;""===c&&(c=b.location.href);return unescape(c.replace(new RegExp("^(?:.*[&\\?]"+escape(a).replace(/[\.\+\*]/g,"\\$&")+"(?:\\=([^&]*))?)?.*$","i"),"$1"))},createJSDODataSource:function(a,c){try{var e=b.spark.createJSDO(a);c.onBeforeFill&&(e.subscribe("beforeFill",c.onBeforeFill),delete c.onBeforeFill);
c.onAfterSaveChanges&&(e.subscribe("afterSaveChanges",c.onAfterSaveChanges),delete c.onAfterSaveChanges);var g=e.getMethodProperties("read"),h=[];g&&g.capabilities&&(h=g.capabilities.split(","));var n={serverFiltering:-1<h.indexOf("filter"),serverPaging:-1<h.indexOf("skip")&&-1<h.indexOf("top"),serverSorting:-1<h.indexOf("orderBy")||-1<h.indexOf("sort"),transport:{jsdo:e},type:"jsdo",error:b.spark.jsdoError};c.tableRef&&(e._buffers&&e._buffers[c.tableRef]?n.transport.tableRef=c.tableRef:console.log("Warning: Requested table '"+
c.tableRef+"' does not exist in JSDO buffers."),delete c.tableRef);var m=new f.data.DataSource(d.extend(n,c)),k=[];m.getFieldSchema=function(a){if(0===k.length){var b=(m.transport||{}).jsdo||null;k=b?b.getSchema():[]}return d.grep(k,function(b){return b.title==a})[0]||{}};return m}catch(q){return console.log("Unable to create JSDO DataSource for "+a+":",c),console.log(q.stack),new f.data.DataSource}},jsdo_read:function(a,c){var d=null,d=c.filterQuery||"",d=""===d?a.fill():a.fill(d);d.then(function(a){},
b.spark.jsdoFailure);return d},jsdo_create:function(a,c){a.add(c.data);var d=a.saveChanges(c.useSubmit);d.then(function(a){},b.spark.jsdoFailure);return d},jsdo_update:function(a,c){var d={},d=c.tableRef&&a[c.tableRef]?a[c.tableRef].findById(c.id):a.findById(c.id);try{d.assign(c.data)}catch(p){c.error(null,null,p)}d=a.saveChanges(c.useSubmit);d.then(function(){},b.spark.jsdoFailure);return d},jsdo_delete:function(a,c){var d=a.findById(c.id);try{d.remove()}catch(p){c.error(null,null,p)}d=a.saveChanges(c.useSubmit);
d.then(function(){},b.spark.jsdoFailure);return d}}}})(window.jQuery,window.kendo,window.progress,window);
(function(d,f){d&&f&&"undefined"!==typeof window.spark&&(window.spark=d.extend(window.spark,{getUrlParams:function(){var a={},b=location.search.substring(1)||"";""==b&&(b=location.hash||"",-1<b.indexOf("?")&&(b=b.split("?")[1]));b&&-1<b.indexOf("=")&&(a=b.replace(/&/g,'","').replace(/=/g,'":"'),a=JSON.parse('{"'+a+'"}',function(a,b){return""===a?b:decodeURIComponent(b)}));return a},clearPersistentObject:function(a){var b=window.localStorage||null;b&&b.clearObject?b.clearObject(a):b?delete b[a]:window.spark.setCookie(a,
"");return!0},setPersistentObject:function(a,b){var c=window.localStorage||null;c&&c.getObject?c.setObject(a,b):c?c[a]=JSON.stringify(b):window.spark.setCookie(a,JSON.stringify(b));return!0},getPersistentObject:function(a){var b="",c=window.localStorage||null;if(c&&c.getObject)return c.getObject(a);if(c){if(a=c[a],"string"===typeof a)try{b=JSON.parse(a)}catch(e){}}else if(a=window.spark.getCookie(a),"string"===typeof a)try{b=JSON.parse(a)}catch(e){}return b},clearSessionObject:function(a){var b=window.sessionStorage||
null;b&&b.clearObject?b.clearObject(a):b&&delete b[a];return!1},setSessionObject:function(a,b){var c=!1,d=window.sessionStorage||null;d&&d.getObject?(d.setObject(a,b),c=!0):d&&(d[a]=JSON.stringify(b),c=!0);return c},getSessionObject:function(a){var b="",c=window.sessionStorage||null;if(c&&c.getObject)b=c.getObject(a);else if(c){var d=c[a];"string"===typeof c[a]&&(b=JSON.parse(d))}return b},getCookie:function(a){a+="=";var b,c=document.cookie.split(";"),d;for(b=0;b<c.length;b+=1){for(d=c[b];" "==d.charAt(0);)d=
d.substring(1);if(0==d.indexOf(a))return d.substring(a.length,d.length)}return""},setCookie:function(a,b,c){var d=new Date;d.setTime(d.getTime()+864E5*(c||1));c="expires="+d.toUTCString();document.cookie=a+"="+b+"; "+c+"; path=/"},getToday:function(){var a=new Date;return new Date(a.getFullYear(),a.getMonth(),a.getDate(),0,0,0,0)},getDateByDays:function(a){if("number"==typeof a&&0!=a){var b=new Date;a=b.setDate(b.getDate()+a);return new Date(a)}return new Date},getDateByWeeks:function(a){if("number"==
typeof a&&0!=a){var b=new Date,c=this.getDateByDays(-1*b.getDay()),c=b.setDate(c.getDate()+7*a);return new Date(c)}return new Date},getTemplate:function(a,b){if(d(a).length){var c=f.template(d(a).html());return b?c(b):c}return null}}));window.Storage&&(Storage.prototype.clearObject=function(a){this.removeItem&&this.removeItem(a)},Storage.prototype.getObject=function(a){return this.getItem?JSON.parse(this.getItem(a)):""},Storage.prototype.setObject=function(a,b){if(this.setItem)try{var c=JSON.stringify(b);
this.setItem(a,c)}catch(e){console.warn("Unable to store peristent object in DOM: "+e),console.warn("An error was encountered while storing data to the browser. Please view the ISC logs for details.")}})})(window.jQuery,window.kendo);
(function(d,f){d&&f&&window.spark&&(window.spark.loader={loadExtScreen:function(a,b,c){return d.ajax({dataType:"script",url:a+".js",success:function(c){return d.ajax({dataType:"html",url:a+".html",success:function(a){f.destroy(d("#"+b).children());d("#"+b).empty();d("#"+b).html(a)},error:function(b,c,d){404===b.status?alert("Error locating HTML file "+a+".html"):window.location.href=window.loginScreenURI?window.loginScreenURI:"/"}})},error:function(b,c,d){404===b.status?alert("Error locating JavaScript file "+
a+".js"):window.location.href=window.loginScreenURI?window.loginScreenURI:"/"}})},loadExtInclude:function(a,b,c,e){return d.ajax({dataType:"html",url:a,success:function(a){f.destroy(d("#"+b).children());d("#"+b).empty();d("#"+b).html(a);d("[data-toggle=tooltip]").tooltip()},error:function(b){alert("Error loading template file "+a)}})},loadExtTemplate:function(a,b,c,e){return d.ajax({dataType:"html",url:a,success:function(a){d("#"+b).append(a);a=f.template(d("#"+c).html())(e||{});f.destroy(d("#"+b).children());
d("#"+b).empty();d("#"+b).html(a)},error:function(b){alert("Error loading template file "+a)}})}})})(window.jQuery,window.kendo);
...
"function"===typeof a.callback&&a.callback(b,c);return a.returnVal||!1}},createReadOnlyEditor:function(){return function(a,b){a.text(b.model[b.field])}},createDatePickerEditor:function(a){return function(b,c){c.sparkEditor=window.spark.field.createDatePicker(d('<input data-bind="value:'+c.field+'"/>').appendTo(b),a);c.sparkEditor.grid=d(b).closest("div.k-grid").getKendoGrid();setTimeout(function(){c.sparkEditor.value(c.model[c.field]||c.model.defaults[c.field])},20)}},createFormattedFieldEditor:function(a){return function(b,
c){c.sparkEditor=window.spark.field.createFormattedField(d('<input data-bind="value:'+c.field+'"/>').appendTo(b),a);c.sparkEditor.grid=d(b).closest("div.k-grid").getKendoGrid();setTimeout(function(){c.sparkEditor.value(c.model[c.field]||c.model.defaults[c.field])},20)}},createMultiLookup:function(a){return function(b,c){c.sparkEditor=window.spark.field.createMultiLookup(d('<input data-bind="value:'+c.field+'"/>').appendTo(b),a);c.sparkEditor.grid=d(b).closest("div.k-grid").getKendoGrid();setTimeout(function(){var a=
c.model[c.field]||c.model.defaults[c.field];"string"===typeof a&&-1<a.indexOf(",")&&(a=a.split(","));c.sparkEditor.value(a)},20)}},createSimpleLookupEditor:function(a){return function(b,c){c.sparkEditor=window.spark.field.createSimpleLookup(d('<input data-bind="value:'+c.field+'"/>').appendTo(b),a);c.sparkEditor.grid=d(b).closest("div.k-grid").getKendoGrid();setTimeout(function(){c.sparkEditor.value(c.model[c.field]||c.model.defaults[c.field])},20)}},createSingleLookupEditor:function(a){return function(b,
c){c.sparkEditor=window.spark.field.createSingleLookup(d('<input data-bind="value:'+c.field+'"/>').appendTo(b),a);c.sparkEditor.grid=d(b).closest("div.k-grid").getKendoGrid();setTimeout(function(){c.sparkEditor.value(c.model[c.field]||c.model.defaults[c.field])},20)}},createInvokeLookupEditor:function(a){return function(b,c){c.sparkEditor=window.spark.field.createInvokeLookup(d('<input data-bind="value:'+c.field+'"/>').appendTo(b),a);c.sparkEditor.grid=d(b).closest("div.k-grid").getKendoGrid()}},
createResourceLookupEditor:function(a){return function(b,c){c.sparkEditor=window.spark.field.createResourceLookup(d('<input data-bind="value:'+c.field+'"/>').appendTo(b),a);c.sparkEditor.grid=d(b).closest("div.k-grid").getKendoGrid()}}})})(window.jQuery,window.kendo);
...
(function(d,f){d&&f&&window.spark&&(window.spark.notify={setNotificationArea:function(a,b){var c=null,e=d(a);e.length&&(c=e.kendoNotification(d.extend({appendTo:a,autoHideAfter:3E4,button:!0},b)).getKendoNotification(),c.showNotification=function(a,b){var c=this;if(c)try{if("string"===typeof a&&""!==a?c.show(a,b||"info"):Array.isArray(a)&&d.each(a,function(a,d){""!==d&&c.show(d,b||"info")}),c.options&&c.options.appendTo){var e=d(c.options.appendTo);e.length&&e.scrollTop(e[0].scrollHeight)}}catch(l){console.log(l)}});
